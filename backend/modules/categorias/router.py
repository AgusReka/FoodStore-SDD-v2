...
async def create_categoria(data: CategoriaCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    repo = CategoriaRepository(db)
    service = CategoriaService(repo)
...
async def update_categoria(categoria_id: UUID, data: CategoriaUpdate, db: Annotated[AsyncSession, Depends(get_db)]):
    repo = CategoriaRepository(db)
    service = CategoriaService(repo)
...
async def delete_categoria(categoria_id: UUID, db: Annotated[AsyncSession, Depends(get_db)]):
    repo = CategoriaRepository(db)
    service = CategoriaService(repo)
